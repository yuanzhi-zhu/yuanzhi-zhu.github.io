...
In this blog, We show how the MeanFlow (backward distillation) identity and the tri-consistency constraint can be combined to produce a *second-order* expression for the student velocity in diffusion/flow distillation. 
Under small-step approximations this reduces to a remarkably simple and elegant target: the average of two end point velocities and a second-order correction term. 
...
1. **MeanFlow (backward) identity** (a local backward distillation relation): for $s > t$,
   $$
...
2. **Tri-consistency** (additivity of short segments): for $s_2 > s_1 > t$,
   $$
   x_t + (s_1-t)\,v(x_t,t,s_1) + (s_2-s_1)\,v(x_{s_1},s_1,s_2)
   \;=\;
   x_t + (s_2-t)\,v(x_t,t,s_2).
   $$
   This says that two short jumps ($t$ to $s_1$ to $s_2$) should equal the single jump (from $t$ to $s_2$).

**Goal:** derive a practical target for $v_\theta(x_t,t,s)$ that combines forward and backward constraints.

---

## 2. From tri-consistency to a forward relation ($s_1$ approach $s_2$)

Start from tri-consistency and replace the second segment by a teacher simulation. Use the teacher to simulate from $s_1$ to $s_2$:
$$
...
This exact identity still contains mixed derivatives and is not an immediately usable loss. 
But we can substitute the MeanFlow identity to eliminate the first time derivative and rearrange to isolate $v(x_t,t,s)$, yields the **second-order MeanFlow formula**:
